...
export default function PredictedPage() {
  ...
  return (
    <PageShell>
      {!bundle ? (
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3" role="status" aria-label="Loading lineups">
      ...
      <header className="mb-6">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          >
            {allTeams.map((code) => (
              <ToggleGroupItem key={code} value={code} aria-label={`Filter to ${code}`}>
                {code}
              </ToggleGroupItem>
            ))}
          </ToggleGroup>
          {selectedTeams.length > 0 ? (
            <Button variant="ghost" size="sm" onClick={() => setSelectedTeams([])} aria-label="Clear team filter">
              Clear
            </Button>
          ) : null}
        </div>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
        {teams.map(({ teamId, teamName, players }) => {
          const ordered = players
            ...
            .slice(0, 11)
          return (
            <Card key={teamId} size="flush" className="p-4" role="region" aria-labelledby={`team-${teamId}-heading`}>
              <h2 id={`team-${teamId}-heading`} className="mb-3 otm-title text-lg">
                {teamName}{" "}
              ...
              <ol className="space-y-2">
                {ordered.map((p) => (
                  <li key={p.id} className="flex items-center gap-2 border-b border-border pb-2 text-sm last:border-b-0">
                    <img src={p.images.avatar ?? undefined} alt="" className="h-5 w-5 rounded-full" />
                    <span>{p.name}</span>
